...
public class DurationFormatterTest {
    ...
    public void formatterTest0M() {
        ...
        assertEquals("0m", DurationFormatter.format(duration));
    }

    @Test
    public void formatterTest30M() {
        Duration duration = Duration.ofMinutes(30);
        assertEquals("30m", DurationFormatter.format(duration));
    }

    @Test
    public void formatterTest60M() {
        Duration duration = Duration.ofMinutes(60);
        assertEquals("1h", DurationFormatter.format(duration));
    }

    @Test
    public void formatterTest90M() {
        Duration duration = Duration.ofMinutes(90);
        assertEquals("1h 30m", DurationFormatter.format(duration));
    }

    @Test
    public void formatterTest120M() {
        Duration duration = Duration.ofMinutes(120);
        assertEquals("2h", DurationFormatter.format(duration));
    }

    @Test
    public void formatterTest8H() {
        Duration duration = Duration.ofMinutes(8 * 60);
        assertEquals("8h", DurationFormatter.format(duration));
    }

    @Test
    public void formatterTest24H() {
        Duration duration = Duration.ofMinutes(24 * 60);
        assertEquals("24h", DurationFormatter.format(duration));
    }

    @Test
    public void formatterTest40H() {
        Duration duration = Duration.ofMinutes(40 * 60);
        assertEquals("40h", DurationFormatter.format(duration));
    }
